...
if (add_open)
{ //если окно карточки по наведению открыто

	for (let i = 0; i < len_green; i++)
	{ //смотреть все карточки
		add_open[i].onclick = function (evt)
		{ //по клику на зеленую кнопку
			evt.preventDefault();
	...
	for (let i = 0; i < len_green; i++)
	{ //смотреть все карточки
		button_book[i].onclick = function (evt)
		{ //по клику на зеленую кнопку
			evt.preventDefault();
